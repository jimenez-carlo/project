...
if (in_array($page, $pages)) {
  ...
  switch ($page) {
    ...
    case 'admin/project/my_list':
      ...
      $where = ($_SESSION['user']->access_id == "2") ? " AND find_in_set({$_SESSION['user']->id}, p.personell_ids) " : "";
      $where_status = (isset($_GET['status_id'])) ? " AND p.status_id = {$_GET['status_id']} " : "";

      $data['list'] = $base->get_list("select concat(o.last_name, ', ', o.first_name,' ', LEFT(o.middle_name, 1), '[#',o.id,']') as officer_full_name, p.id,s.name as `status`,ui.name as `implementing_unit`,cm.name as`comodity`,pm.name as `program_manager`, p.project_description, p.qty, u.name AS `unit`, p.abc, p.contract_price, p.gaa, p.created_date,p.updated_date,epa from 
...
tbl_users_info c on c.id = p.created_by where p.deleted_flag = 0");
    ...
    case 'admin/report/commodity':
      $select = [];
      $commodities = $base->get_list("SELECT id FROM tbl_comodity WHERE deleted_flag = 0");
      $is_maker = ($_SESSION['user']->access_id == "2") ? " AND find_in_set({$_SESSION['user']->id}, p.personell_ids) " : "";
      $where_gaa = (isset($_GET['gaa'])) ? " AND p.gaa = UPPER('".$_GET['gaa']."') " : "";
      ...
      if (count($commodities) > 0) {
        foreach ($commodities as $commodity) {
            ...
            $select[] = "
              SELECT 
              c.name AS `Commodity`,
              SUM(IF(status_id = 1  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `For Pre-Proc`,
              SUM(IF(status_id = 2  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Pre-Proc Passed`,
              SUM(IF(status_id = 3  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Pre-Proc Failed`,
              SUM(IF(status_id = 4  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Pre-Bid`,
              SUM(IF(status_id = 5  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Sobe Passed`,
              SUM(IF(status_id = 6  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Sobe Failed`,
              SUM(IF(status_id = 7  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `PQ Passed`,
              SUM(IF(status_id = 17 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `PQ Failed`,
              SUM(IF(status_id = 8  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `PQR`,
              SUM(IF(status_id = 9  AND p.comodity_id = {$commodity_id}, 1, 0)) AS `NOA`,
              SUM(IF(status_id = 10 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `ORS`,
              SUM(IF(status_id = 11 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `NTP`,
              SUM(IF(status_id = 12 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `NTP Conforme`,
              SUM(IF(status_id = 13 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Delivery`,
              SUM(IF(status_id = 14 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Inspected`,
              SUM(IF(status_id = 15 AND p.comodity_id = {$commodity_id}, 1, 0)) AS `Accepted`,
              COUNT(*) AS `Total`
              FROM tbl_project p 
              INNER JOIN tbl_comodity c ON c.id = p.comodity_id
              WHERE p.deleted_flag = 0 AND p.comodity_id = {$commodity_id} {$where_gaa} {$is_maker}
              GROUP BY p.comodity_id
            ";
        }
        $select_union_commodities = implode(" UNION ", $select);
        $data['commodities'] = $base->get_list("SELECT * FROM ( {$select_union_commodities} ) AS commodity_status_report ");
      }
      break;
  }
